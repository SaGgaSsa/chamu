...
pub fn convert_interleaved_to_mono_16k(input: &[f32], channels: u16, sample_rate: u32) -> Vec<i16> {
    ...
    let output_len =
        ((mono.len().saturating_sub(1) as u64 * 16_000) / sample_rate as u64 + 1) as usize;
    (0..output_len)
        .map(|index| {
            ...
            let sample =
                mono[before] + (mono[after] - mono[before]) * (position - before as f64) as f32;
    ...
    let normalized = input
        ...
        .map(|sample| {
            if *sample == u16::MAX {
                ...
                (*sample as f32 - 32_768.0) / 32_767.0
            }
        })
        .collect::<Vec<_>>();
    convert_interleaved_to_mono_16k(&normalized, channels, sample_rate)
...
pub fn convert_interleaved_f32_to_mono_16k(
    input: &[f32],
    channels: u16,
    sample_rate: u32,
) -> Vec<i16> {
...
pub fn convert_interleaved_i8_to_mono_16k(
    input: &[i8],
    channels: u16,
    sample_rate: u32,
) -> Vec<i16> {
    convert_interleaved_to_mono_16k(
        &input.iter().map(|v| *v as f32 / 128.0).collect::<Vec<_>>(),
        channels,
        sample_rate,
    )
}

pub fn convert_interleaved_u8_to_mono_16k(
    input: &[u8],
    channels: u16,
    sample_rate: u32,
) -> Vec<i16> {
...
pub fn convert_interleaved_i64_to_mono_16k(
    input: &[i64],
    channels: u16,
    sample_rate: u32,
) -> Vec<i16> {
    convert_interleaved_to_mono_16k(
        &input
            .iter()
            .map(|v| *v as f64 as f32 / i64::MAX as f32)
            .collect::<Vec<_>>(),
        channels,
        sample_rate,
    )
...
pub fn convert_interleaved_u64_to_mono_16k(
    input: &[u64],
    channels: u16,
    sample_rate: u32,
) -> Vec<i16> {
    convert_interleaved_to_mono_16k(
        &input
            .iter()
            .map(|v| (*v as f64 - (u64::MAX as f64 / 2.0)) as f32 / (u64::MAX as f64 / 2.0) as f32)
            .collect::<Vec<_>>(),
        channels,
        sample_rate,
    )
...
mod tests {
    ...
    fn conversion_averages_interleaved_stereo_to_mono() {
        let samples = convert_interleaved_to_mono_16k(&[1.0_f32, -1.0, 0.5, 0.5], 2, 16_000);
    ...
    fn conversion_supports_signed_and_unsigned_device_samples() {
        assert_eq!(
            super::convert_interleaved_i16_to_mono_16k(&[i16::MIN, 0, i16::MAX], 1, 16_000,),
